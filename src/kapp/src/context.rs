...
pub struct Context<'a> {
    ...
    /// 上一帧的聚簇统计：`(每簇平均几盏, 最长的那个簇有几盏, 溢出条目数)`。
    ///
    /// 平均值说明聚簇有没有在干活；**最坏情况才决定帧时间**——
    /// GPU 按 warp 走，一个 warp 里最慢的那个像素拖着所有人。
    /// 溢出不为 0 说明有簇的名单满了，画面上某些地方少了几盏灯。
    pub cluster_stats: (f32, u32, u32),
    ...
    /// 聚簇前向着色。改了下一帧生效。
    ///
    /// **默认开着**：它是「场景里能有几百盏灯」的前提。关掉之后每个片元
    /// 遍历全部光源，几十盏以上就会明显掉帧。
    ///
    /// 正交相机下自动退回全遍历——深度切片按 z 取对数，那个公式在正交
    /// 投影下不成立。正交基本只用在 2D 和编辑器视图上，那里灯本来就没几盏。
    pub clusters: &'a mut krender::ClusterSettings,
    /// 屏幕空间环境光遮蔽。改了下一帧生效。
    ///
    /// **默认关着**：它要把整个场景的几何多渲一遍（深度 + 法线预通道），
    /// 而大多数场景没有它也说得过去——不像软粒子那样是「不开就明显穿帮」。
    ///
