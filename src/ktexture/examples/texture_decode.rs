...
fn main()->Result<(),Box<dyn std::error::Error>>{
    ...
    if args.len()!=2 {return Err("usage: texture_decode INPUT OUTPUT.rgba".into());}
    let manager=ResourceManager::new();manager.add_loader(TextureLoader);
    ...
    output.extend(texture.width().to_le_bytes());output.extend(texture.height().to_le_bytes());output.extend(texture.data());
    std::fs::write(&args[1],output)?;Ok(())
}
