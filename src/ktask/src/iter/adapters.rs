...
// 本文件定义的这些适配器结构体（Chain/Map/Filter/...）都对应 std::iter 里同名的适配器概念，
// 但作用在"批次"级别，而不是单个元素级别：每个适配器包一层 next_batch，
// 在拿到内层的 batch（一个普通 std Iterator）之后，用 std 自带的 .map()/.filter() 等方法
// 把批次本身转换成对应的新迭代器类型，真正的逐元素处理仍然发生在 batch 被消费（如 s.spawn 内部跑）的时候。
