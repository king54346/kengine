...
/// workloads. In particular, if the batch size is too small or ktask being
...
// ParallelIterator 的核心思路：不是逐个元素并行（那样开销太大），而是"按批次(batch)并行"。
// next_batch() 每次吐出一批数据（一个普通的 std Iterator），
// 各个消费方法（如 count/map/collect）内部用 pool.scope 给每一批 spawn 一个任务，
// 多批数据在多个线程上同时被处理，最后再合并结果。
    ...
    Self: Sized + Send,
{
    ...
    // 由具体实现（比如某个基于 slice::chunks 的类型）决定如何切分批次；
    // 本 trait 里其余所有方法都是基于 next_batch 实现的默认方法。
    ...
    fn count(mut self, pool: &TaskPool) -> usize {
        // 典型写法：不断取出下一批，每批各自 spawn 一个任务去数数，
        // scope() 会阻塞直到所有批次都数完，返回各批次数量的 Vec，最后求和得到总数。
        pool.scope(|s| {
    ...
    where
        F: FnMut(BatchIter::Item) -> bool + Send + Sync + Clone,
    {
        // 每个批次各自查找命中位置（批内相对下标）和批次长度，不做短路（所有批次都会跑完）；
        // 收集完之后再按批次顺序把"批内下标"换算成"全局下标"（累加之前所有批次的长度作为偏移 start）。
