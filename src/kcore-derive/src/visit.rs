...
pub fn impl_visit(ast: DeriveInput) -> TokenStream {
    ...
    match &ty_args.data {
        ...
        ast::Data::Enum(variants) => self::impl_visit_enum(&ty_args, variants),
    }
}
...
fn impl_visit_struct(
    ...
    let visit_fn_body = if field_args.style == ast::Style::Unit {
        ...
        fn make_method_call(name: Option<&Ident>) -> TokenStream {
            match name {
                None => {
                    quote! { (); }
                }
...
/// impl `Visit` for `enum`
fn impl_visit_enum(ty_args: &args::TypeArgs, variant_args: &[args::VariantArgs]) -> TokenStream {
